...
int main (void)
{
	...
	DDRD = 0x20;				// bidirectional switch
	PORTD = 0xFF;
	DDRB = 0xFF;
	PORTB = 0x00;
...
ISR (TIMER1_OVF_vect)				//ISR FOR TIMER1 COMPARE MATCH A
{
	TCNT1H = (-65530)>>8;     // THE HIGH BYTE
	TCNT1L = (-62530)&0XFF;	  //OVERFLOW AFTER THIS MANY CLOCK CYCLES
	if (CHARGE < 0x5500 || VOLT < 0x3B00)			// 0x3B --> 2.73 Volt/Cell    --> 5.45 volts/battery
	{
		PORTD = 0x00;
	}

	else if (CHARGE > 0xEFFF || VOLT > 0x5900)		// 0x5A --> 4.14 Volt/Cell   --> 8.27 volt/battery
	{
		PORTD = 0x00;
	}
	else
